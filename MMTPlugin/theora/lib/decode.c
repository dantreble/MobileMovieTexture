...
static int oc_dec_init(oc_dec_ctx *_dec,const th_info *_info,
 const th_setup_info *_setup, int _pot){
  int qti;
  int pli;
  int qi;
  int ret;
  ret=oc_state_init(&_dec->state,_info,3,_pot);
  if(ret<0)return ret;
...
th_dec_ctx *th_decode_alloc(const th_info *_info,const th_setup_info *_setup, int _pot){
  oc_dec_ctx *dec;
  if(_info==NULL||_setup==NULL)return NULL;
  dec=oc_aligned_malloc(sizeof(*dec),16);
  if(dec==NULL||oc_dec_init(dec,_info,_setup,_pot)<0){
    oc_aligned_free(dec);
    return NULL;
  }
